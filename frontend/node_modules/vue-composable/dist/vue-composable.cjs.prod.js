...
function unwrap(o) {
    return runtimeCore.isRef(o) ? o.value : o;
    // return unref(o) as T;
}
function wrap(o) {
    return runtimeCore.isRef(o) ? o : runtimeCore.ref(o); // NOTE in v3 this is not necessary
}
...
function useEvent(el, name, listener, options) {
    ...
    if (el) {
        ...
        const removeWatch = runtimeCore.watch(element, (n, _, cleanUp) => {
            if (n) {
                addEventListener(n);
...
function useOnMouseMove(el, options, wait) {
    const mouseX = runtimeCore.ref(0);
    const mouseY = runtimeCore.ref(0);
    const pageX = runtimeCore.ref(0);
    const pageY = runtimeCore.ref(0);
    let handler = (ev) => {
        mouseX.value = ev.x;
        mouseY.value = ev.y;
...
function useOnResize(el, options, wait) {
    const element = wrap(el);
    const height = runtimeCore.ref(element.value && element.value.clientHeight);
    const width = runtimeCore.ref(element.value && element.value.clientWidth);
    let handler = () => {
    ...
    if (runtimeCore.isRef(el) && !el.value) {
        runtimeCore.onMounted(handler);
    }
    return {
        height,
        width,
...
function useOnScroll(el, options, wait) {
    const isValidElement = (el) => isNumber(el) || isBoolean(el) || !(isElement(el) || runtimeCore.isRef(el)) || !el
        ? false
        : true;
    const element = isValidElement(el)
        ? wrap(el)
        : runtimeCore.ref((isClient && window) || undefined);
    const scrollableElement = isValidElement(el)
        ? element
        : runtimeCore.ref((isClient && window.document.scrollingElement) || undefined);
    const scrollTop = runtimeCore.ref((scrollableElement.value && scrollableElement.value.scrollTop) || 0);
    const scrollLeft = runtimeCore.ref((scrollableElement.value && scrollableElement.value.scrollLeft) || 0);
    let handler = () => {
    ...
    const watchRemoveTop = runtimeCore.watch(scrollTop, scrollTopTo, { immediate: false });
    const watchRemoveLeft = runtimeCore.watch(scrollLeft, scrollLeftTo, { immediate: false });
    const remove = () => {
        eventRemove();
        watchRemoveLeft();
        watchRemoveTop();
...
function usePromise(fn, lazyOptions) {
    ...
    const [lazy, throwException] = isBoolean(lazyOptions)
        ...
        : isObject(lazyOptions)
            ...
            : [false, false];
    const loading = runtimeCore.ref(false);
    const error = runtimeCore.ref(null);
    const result = runtimeCore.ref(null);
    const promise = runtimeCore.ref();
    const exec = async (...args) => {
        ...
        try {
            ...
            if (runtimeCore.toRaw(promise.value) === runtimeCore.toRaw(currentPromise)) {
                error.value = er;
                result.value = null;
            }
...
function useCancellablePromise(fn, lazyOptions) {
    ...
    if (runtimeCore.getCurrentInstance() &&
        (!isObject(lazyOptions) || lazyOptions.unmountCancel !== false)) {
        runtimeCore.onUnmounted(() => _cancel && _cancel());
    }
    return {
        ...use,
        cancel,
...
const ExecutionId = /*#__PURE__*/ Symbol(``);
/* istanbul ignore next */
...
function useRetry(options, factory) {
    ...
    const isRetrying = runtimeCore.ref(false);
    const nextRetry = runtimeCore.ref();
    const retryErrors = runtimeCore.ref([]);
    const cancellationToken = { value: false };
    const retryId = { value: 0 };
    const retryCount = runtimeCore.computed(() => retryErrors.value.length);
    const context = {
        isRetrying,
        retryCount,
        nextRetry,
...
function getDevtools() {
    const empty = {};
    const devtools = runtimeCore.inject(DEVTOOLS_KEY, empty);
    if (devtools === empty) {
...
function useDevtoolsInspector(options, nodeList = []) {
    ...
    const nodes = runtimeCore.ref(nodeList);
    const byId = runtimeCore.computed(() => {
        if (!nodes.value)
            return new Map();
        const r = runtimeCore.toRaw(nodes.value);
        const m = new Map();
        for (const i of r) {
            m.set(i.id, i);
        }
    ...
    if (api) {
        ...
        api.on.getInspectorTree((payload) => {
            ...
            let m = runtimeCore.toRaw(nodes.value);
            if (payload.filter) {
        ...
        api.on.getInspectorState((payload) => {
            ...
            if (node) {
                const s = runtimeCore.reactive(node.state); // unwrap
                payload.state = options.stateFilter ? options.stateFilter("", s) : s;
            }
        });
        runtimeCore.watch(nodes, () => {
...
function useFetch(options, requestInitOptions) {
    ...
    const json = runtimeCore.ref(null);
    const text = runtimeCore.ref("");
    const blob = runtimeCore.ref();
    const jsonError = runtimeCore.ref(null);
        ...
        ? isString(options)
            ...
            : isFetchOptions(options)
                ...
                : isString(options.url)
                    ...
                    : options
        : undefined;
    const isCancelled = runtimeCore.ref(false);
    const cancelledMessage = runtimeCore.ref();
    ...
    const status = runtimeCore.computed(() => (use.result.value && use.result.value.status) || null);
    const statusText = runtimeCore.computed(() => (use.result.value && use.result.value.statusText) || null);
    ...
    if (unmountCancel && runtimeCore.getCurrentInstance()) {
...
function useWebSocket(url, protocols) {
    ...
    let ws = null;
    const messageEvent = runtimeCore.ref(null);
    const errorEvent = runtimeCore.ref(null);
    const data = runtimeCore.ref(null);
    const isOpen = runtimeCore.ref(false);
    const isClosed = runtimeCore.ref(false);
    const errored = runtimeCore.ref(false);
    let send = NO_OP;
    let close = NO_OP;
    if (supported) {
...
function useIntersectionObserver(refEl, refOptions) {
    ...
    const element = wrappedElement && (isElement(wrappedElement.value) || !wrappedElement.value)
        ? wrappedElement
        : undefined;
    const options = runtimeCore.computed(() => refOptions
        ? unwrap(refOptions)
        : !element
            ? unwrap(refEl)
            : undefined);
    const elements = runtimeCore.ref([]);
    const isIntersecting = runtimeCore.computed(() => elements.value.length > 0 && elements.value.every(x => x.isIntersecting));
    const handling = (entries) => {
        elements.value = entries;
    };
    let observer = runtimeCore.ref();
    if (supported) {
        runtimeCore.watch(options, options => {
            if (observer.value) {
    ...
    //     false && console.warn('[IntersectionObserver] no elements provided, did you mount the component?')
    //     return;
    //   }
...
function usePageVisibility() {
    if (!hidden) {
        hidden = runtimeCore.ref(isClient && document.hidden);
    }
    if (!visibility) {
        if (isClient) {
            ...
            visibility = runtimeCore.ref("visible");
        }
    }
    return {
        visibility,
...
function useLanguage() {
    if (!language) {
        language = isClient ? runtimeCore.ref(navigator.language) : runtimeCore.ref("");
    }
    if (!languages) {
        if (isClient) {
...
function useBroadcastChannel(name, onBeforeClose) {
    ...
    if (supported) {
        ...
        addListener = (cb, o) => {
            ...
            runtimeCore.onUnmounted(() => bc.removeEventListener("message", cb));
        };
        runtimeCore.onUnmounted(() => {
...
function useGeolocation(options) {
    ...
    const lazy = runtimeCore.ref(options ? options.immediate === false : undefined);
    const error = runtimeCore.ref(null);
    const timestamp = runtimeCore.ref(null);
    const coords = runtimeCore.ref(null);
    const highAccuracy = runtimeCore.ref((options && options.enableHighAccuracy) || null);
    ...
    if (supported) {
        ...
        let watchId = 0;
        runtimeCore.onMounted(() => runtimeCore.watch([highAccuracy, lazy], (a) => {
            clearWatch();
        ...
        runtimeCore.onUnmounted(clearWatch);
    }
    return {
        supported,
        refresh,
...
function useCssVariables(variables, elementOrOptions, optionsConfig) {
    ...
    const [element, options] = runtimeCore.isRef(elementOrOptions) || isElement(elementOrOptions)
    ...
    const observing = runtimeCore.ref(true);
    ...
    for (let i = 0; i < defEntries.length; i++) {
        ...
        if (!result[key]) {
            ...
            result[key] = runtimeCore.ref((runtimeCore.isRef(element) && !element.value) || !supported
                ? null
                : getCssVariableFor(unwrap(element), name));
        }
        if (supported) {
            // keep track of changes
            runtimeCore.watch([result[key], wrap(element)], (val) => {
                if (!observing)
                    return;
            ...
            }, { immediate: !runtimeCore.isRef(element) });
        }
    }
    if (!supported) {
        return {
    ...
    // Stops on destroy
    runtimeCore.onUnmounted(stop);
    if (runtimeCore.isRef(element)) {
        runtimeCore.onMounted(() => {
            updateValues.forEach(x => x());
            runtimeCore.watch(element, (n, o) => {
                if (o) {
                    stop();
                }
                if (n) {
...
function useWorker(stringUrl, args, options) {
    ...
    const errorEvent = runtimeCore.ref();
    const data = runtimeCore.ref();
    const terminated = runtimeCore.ref(!supported);
    const errored = runtimeCore.ref(!supported);
    if (!supported) {
    ...
    runtimeCore.onUnmounted(terminate);
    if (args) {
        postMessage(args);
    }
    return {
...
function useClipboard() {
    ...
    if (supported) {
        ...
        runtimeCore.watch(text, debounce((s) => {
...
function useTimeout(fn, delay = 0) {
    let ready = runtimeCore.ref(false);
    ...
    runtimeCore.onUnmounted(cancel);
    return {
        ready,
        cancel,
    };
...
function useWorkerFunction(fn, options) {
    const supported = isClient && "Worker" in self;
    // reactive
    const dependencies = runtimeCore.computed(() => (options && unwrap(options.dependencies)) || []);
    const timeoutRef = runtimeCore.computed(() => options && unwrap(options.timeout));
    if (!supported) {
    ...
    const promise = useCancellablePromise((...args) => new Promise((res, rej) => {
        ...
        const watchCancel = args.length === fn.length + 1 &&
            runtimeCore.isRef(args[args.length - 1]) &&
            args[args.length - 1].value === false
            ? runtimeCore.computed(() => promise.cancelled.value || args[args.length - 1].value)
            : promise.cancelled;
        const removeWatch = watchCancel
            ? runtimeCore.watch(watchCancel, () => {
                terminate();
                res(undefined);
            }, { immediate: false })
            : NO_OP;
...
function useMatchMedia(query) {
    ...
    let remove = NO_OP;
    if (supported) {
        mediaQueryList = runtimeCore.ref(matchMedia(query));
        matches = runtimeCore.ref(mediaQueryList.value.matches);
        const process = (e) => {
        ...
        mediaQueryList = runtimeCore.ref({});
        matches = runtimeCore.ref(false);
    }
    return {
        supported,
        mediaQueryList,
...
function useSharedRef(name, defaultValue) {
    ...
    const id = Date.now();
    const master = runtimeCore.ref(false);
    const mind = runtimeCore.ref(0 /* HIVE */);
    const editable = runtimeCore.computed(() => mind.value === 1 /* MASTER */ ? master.value : true);
    // who's listening to this broadcast
    const targets = runtimeCore.ref([]);
    const data = runtimeCore.ref(defaultValue);
...
function refShared(defaultValue, id) {
    const vm = runtimeCore.getCurrentInstance();
    const name = id
        ? id
        : vm.vnode.scopeId; // TODO test this :/ NOTE @vue/runtime-core might be different
    if (!name) {
        return runtimeCore.ref(defaultValue);
    }
...
function useLockScroll(selectorElements, classOptions) {
    const elements = (isString(selectorElements)
        ? runtimeCore.ref([]) // it will be resolved when mounted
        : isArray(runtimeCore.unref(selectorElements))
    ...
    const locked = runtimeCore.ref(false);
    // NOTE issues on Vue3 because of the `watch.deep`, we wrap all elements
    const tracked = runtimeCore.computed(() => elements.value.map(x => runtimeCore.ref(x)));
    const remove = runtimeCore.watch([locked, tracked], ([l, els], [oldLocked, oldElements]) => {
    ...
    if (isString(selectorElements)) {
        runtimeCore.onMounted(() => (elements.value = Array.from(document.querySelectorAll(selectorElements))));
    }
    if (options.auto !== false) {
        runtimeCore.onMounted(lock);
        runtimeCore.onActivated(lock);
        runtimeCore.onDeactivated(unlock);
        runtimeCore.onBeforeUnmount(unlock);
    }
    return {
        locked,
        lock,
...
function injectFactory(key, defaultValueFactory) {
    return runtimeCore.inject(key, defaultValueFactory, true);
}
...
function refDebounced(value, delay) {
    let [v, d] = arguments.length === 1 && isNumber(value) && typeof delay === "undefined"
        ? [runtimeCore.ref(), value]
        : [runtimeCore.ref(value), delay];
    return runtimeCore.customRef((track, trigger) => ({
        get() {
            track();
            return v.value;
        },
...
function intlDateFormatExtractArguments(localesOptions, opts) {
    ...
    return isObject(wrappedOpts.value) || runtimeCore.isRef(opts)
        ? [
...
function useIntlDateTimeFormat(localesOptions, opts) {
    ...
    const formatter = runtimeCore.computed(() => new Intl.DateTimeFormat(unwrap(locales), unwrap(options)));
    ...
    const format = (value, overrideOpts, overrideLocale) => runtimeCore.computed(() => formatString(value, overrideOpts, overrideLocale));
    return {
        format,
        formatString,
        formatter,
...
function useIntlNumberFormat(localesOptions, opts) {
    const [locales, options] = intlDateFormatExtractArguments(localesOptions, opts);
    const formatter = runtimeCore.computed(() => new Intl.NumberFormat(unwrap(locales), unwrap(options)));
    const formatString = (value, overrideOpts, overrideLocale) => {
        const f = overrideOpts || overrideLocale
...
function useCurrencyFormat(currencyCodeOptions, localesOptions, opts) {
    ...
    const [locales, argOptions] = intlDateFormatExtractArguments(localesOptions, hasCurrency
        ? opts
        : currencyCodeOptions);
    const options = runtimeCore.computed(() => {
    ...
    const format = (amount, currency, display, opts, overrideLocale) => {
        return runtimeCore.computed(() => formatString(amount, currency, display, opts, overrideLocale));
    };
    return {
        format,
        formatString
...
// istanbul ignore next
const BREAKPOINT_TAILWIND_KEY = Symbol(``);
...
function setBreakpointTailwindCSS(breakpoints) {
    ...
    runtimeCore.provide(BREAKPOINT_TAILWIND_KEY, bp);
    return bp;
}
    ...
    if (supported && storage) {
        if (!store) {
            ...
            store = {
                ...
                getRef(k) {
                    const item = runtimeCore.ref();
                    let keyWatch = runtimeCore.watch(wrap(k), (k) => {
                        const data = storage.getItem(k);
                        if (!data) {
                    ...
                    let valueWatch = runtimeCore.watch(item, debounce((i) => {
                ...
                setItem(k, v) {
                    ...
                    this.save(k, v);
                    // @ts-ignore
                    const stop = runtimeCore.watch(reference, debounce((r) => {
                        this.save(k, r);
                    }, ms), {
                        immediate: false,
                        deep: true,
...
function useLocalStorage(key, defaultValue, sync) {
    ...
    if (supported && store) {
        ...
        storage = runtimeCore.ref(defaultValue);
    }
    return {
        supported,
        storage,
...
function usePagination(options) {
    ...
    runtimeCore.watch([total, pageSize], _ => {
...
function useFormat(format, args) {
    return runtimeCore.computed(() => {
        ...
        const r = runtimeCore.isRef(args) ? runtimeCore.reactive(args.value) : runtimeCore.reactive(args);
        const regEx = /({?{[\w\s]*}?})/g;
...
const buildValidationFunction = (r, f, m, handlers) => {
    const $promise = runtimeCore.ref(null);
    const $pending = runtimeCore.ref(false);
    const $error = runtimeCore.ref();
    const $invalid = runtimeCore.ref(false);
...
const buildValidationValue = (r, v, handlers) => {
    ...
    const { $pending, $promise, $invalid, $error, $touch, } = buildValidationFunction(r, $validator, runtimeCore.ref($message), handlers);
    return {
        $pending,
        $error,
        $promise,
...
const buildValidation = (o, handlers) => {
    ...
    for (const k of Object.keys(o)) {
        if (k[0] === "$") {
            if (k === "$value") {
                r[k] = $value;
                const $dirty = runtimeCore.ref(false);
                let dirtyWatch = NO_OP;
                const createDirtyWatcher = () => {
                    dirtyWatch();
                    dirtyWatch = runtimeCore.watch($value, () => {
                        $dirty.value = true;
                        dirtyWatch();
        ...
        if ($value) {
            ...
            if (isValidation(validation)) {
                const validations = Object.keys(validation)
                    .filter((x) => x[0] !== "$")
                    .map((x) => validation[x]);
                $errors = runtimeCore.computed(() => validations
                    .map((x) => x.$error)
                    .map((x) => unwrap(x))
                    .filter((x) => x !== undefined));
                // $anyDirty = computed(() => validations.some(x => !!x));
                $anyInvalid = runtimeCore.computed(() => validations.some((x) => {
...
function useValidation(input) {
    ...
    const validationInput = runtimeCore.reactive(validation.input);
...
function useI18n(definition) {
    if (definition) {
        ...
        return runtimeCore.inject(I18n_ACCESS_SYMBOL);
}
/**
...
function buildI18n(definition) {
    const locales = runtimeCore.ref(Object.keys(definition.messages));
    const localeMessages = runtimeCore.ref(definition.messages);
    const locale = runtimeCore.ref(definition.locale);
    const i18n = runtimeCore.ref({});
    let fallback = runtimeCore.ref();
    const cache = {};
    ...
    const loadLocale = (locale, messages) => {
        ...
        return (cache[locale] = runtimeCore.computed(() => retrieveLocaleValue(messages.value[locale])));
    };
    ...
    const localeChangesCount = runtimeCore.ref(0);
    runtimeCore.watch(localeMessages, () => localeChangesCount.value++, {
        deep: true,
        immediate: false,
    });
    runtimeCore.watch([locale, fallback, localeChangesCount], ([l, fb, _]) => {
...
function setI18n(definition) {
    ...
    runtimeCore.provide(I18n_ACCESS_SYMBOL, r);
    return r;
}

// istanbul ignore next
const SSR_TITLE_KEY = Symbol(``);
...
function useSSRTitle(defaultTitle) {
    const s = Symbol();
    const title = runtimeCore.inject(SSR_TITLE_KEY, s);
    // @ts-ignore check if it exists
    if (title === s) {
        // istanbul ignore next
        return runtimeCore.ref(isString(defaultTitle) ? defaultTitle : "");
    }
    if (title === undefined) {
...
function useTitle(overrideTitle = null) {
    ...
    const title = runtimeCore.ref(isString(overrideTitle) ? overrideTitle : document.title);
    const observer = new MutationObserver(m => {
        title.value = m[0].target.textContent;
    });
    runtimeCore.watch(title, (t, o) => {
        if (isString(t) && t !== o) {
            document.title = t;
        }
    }, {
    ...
    runtimeCore.onUnmounted(() => observer.disconnect());
    return title;
}

// istanbul ignore next
const HYDRATION_KEY = Symbol(``);
const hydrationPlugin = {
    // @ts-ignore
    install(app) {
        const hydrating = runtimeCore.ref(true);
        const h = runtimeCore.readonly(hydrating);
        {
            // @ts-ignore
...
function useHydration() {
    return runtimeCore.inject(HYDRATION_KEY, runtimeCore.computed(() => false));
}

function useValueSync(main, args) {
    const master = wrap(main);
    const list = arguments.length === 2
        ? runtimeCore.isRef(arguments[1]) && isArray(arguments[1].value)
            ? arguments[1]
            : runtimeCore.ref([wrap(arguments[1])])
        : runtimeCore.ref(Array.from(arguments)
            .slice(1)
            .map(x => wrap(x)));
    ...
    runtimeCore.watch([master, list], ([m, list]) => {
...
function useMouseDistanceFromElement(el, options, wait) {
    ...
    if (isClient) {
        ...
        distance = runtimeCore.computed(() => -1);
    }
    return {
        distance,
        remove,
